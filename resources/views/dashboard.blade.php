<x-app-layout>
    <x-container>
        <form action="{{ route('posts.store') }}" class="px-4 mb-8" method="POST">
            ...
            name="body" 
            rows="2" 
            class="w-full mb-2 p-0 text-white bg-transparent border-0 border-b-2 border-slate-600 focus:border-b-slate-500 focus:ring-0 resize-none overflow-hidden"
        ...
        <a href="{{ route('profile.show', $post->user) }}" class="px-6 mb-2 flex items-center gap-2 font-medium text-slate-100">
            <svg class="h-4" data-slot="icon" aria-hidden="true" fill="currentColor" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
